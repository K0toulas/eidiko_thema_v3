...
def export_to_onnx(model, model_name, features):
    ...
    # Test inference
    sess = ort.InferenceSession(f"{model_name}.onnx")
    input_name = sess.get_inputs()[0].name
    dummy_input = np.zeros((1, len(features)), dtype=np.float32)
    pred = sess.run(None, {input_name: dummy_input})[0]
    print(f"Successfully exported {model_name}.onnx with dummy prediction: {pred}")




# Function to ensure C-contiguous arrays with int32 dtype for tree attributes
def preprocess_rf_model(model):
    model_copy = copy.deepcopy(model)
    for estimator in model_copy.estimators_:
        tree = estimator.tree_
        for attr in ['children_left', 'children_right', 'feature', 'n_node_samples']:
            arr = getattr(tree, attr)
            setattr(tree, attr, np.array(arr, dtype=np.int32, copy=True, order='C'))
        tree.threshold = np.array(tree.threshold, dtype=np.float64, copy=True, order='C')
        tree.value = np.array(tree.value, dtype=np.float64, copy=True, order='C')
    return model_copy
...
def export_to_treelite(model, model_name, features, class_names):
    try:
        # Preprocess model to ensure compatible arrays
        model_processed = preprocess_rf_model(model)
        # Import scikit-learn model into Treelite
        model_treelite = treelite.sklearn.import_model(model_processed)
...
# Export models
export_to_onnx(rf_simple, 'rf_simple', features)
export_to_onnx(rf_step1, 'rf_step1', features)
export_to_onnx(rf_step2, 'rf_step2', features)
